 ...
 *
 * Demonstrates the foundational single-agent pattern using Microsoft Agent Framework GA.
 * Each specialized agent (Architect, Developer, Tester) is created via IChatClient.AsAIAgent()
 * and streams responses with RunStreamingAsync().
 */
...
/// <summary>
/// Single-agent pattern: selects one of three specialized AIAgents based on the requested mode
/// and streams its response back to the caller.
/// </summary>
public class CodingAssistentSession_SingleAgent : ICodingAssistentSession
{
    ...
    public Task InitializeAsync()
    {
        ...
        // Each agent gets its own IChatClient instance so they are independent.
        _architectAgent = AgentDefinitions.CreateArchitectAgent(CreateChatClient());
        _developerAgent = AgentDefinitions.CreateDeveloperAgent(CreateChatClient());
        _testerAgent    = AgentDefinitions.CreateTesterAgent(CreateChatClient());
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if (mode is AssistentModes.DevTeam)
        {
            yield return new AgentMessage(
                "Team mode is not supported in the single-agent implementation. Use Architect, Coder, or Tester mode.",
                "System");
            yield break;
        }
